...
class Api::V1::GroupsController < Api::BaseController
  ...
  def leave
    ...
    if membership.role == 'seeder' && @group.group_memberships.where(role: 'seeder').count == 1
      return render json: { error: 'cannot leave — you are the last seeder. Nominate another first or archive the group.' }, status: 422
    end

...
export const EARTH_CALENDAR: EarthMonth[] = [
  ...
  },
  {
    ...
    observables: [
      'At noon, notice how low the sun sits — shadows fall long even at midday now. The sun is already below 30° at its peak, a dramatic shift from the high overhead summer arc. On clear afternoons the light arrives horizontal and warm, almost at right angles to summer.',
      'On a still morning, look for spider webs between fence posts, silk lit by the low-angle light. The webs are backlit in the first hour after sunrise, making large structures visible in the air that are invisible for the rest of the day. The spiders sit still in the centre, heavy from a season of feeding.',
      'In the Dandenongs or outer suburbs, watch for the first frosts whitening lawns at dawn. Frost forms in hollows and open ground, following cold air drainage patterns that repeat each year. By 9am it is gone — but the burnt tips on tender plants show where it lay.',
      'Listen in the evening for the first boobook owls calling, claiming their winter territory. The soft double call — boo-book, boo-book — is heard from dusk onward in treed suburbs. Sit still in the dark for ten minutes and you may hear several answering across the neighbourhood.',
      'Step outside and smell the air — mornings carry cold damp soil and distant wood smoke. The scent of wood smoke drifting from neighbouring houses is specific to early autumn mornings. Worms have come closer to the surface after first rains; turn a handful of garden soil to find them.',
      'Notice how quickly light fades after 5pm — count the minutes since last week. The rate of change in day length is still noticeable week to week. Check if any day-flying insects are still active in the last half hour of light.',
      'Look for the last deciduous leaves clinging to branches before the wind takes them. Plane trees, elms, and ornamental cherries are at varying stages — some already bare, others still holding on. Silvereyes move through the nearly bare branches, gleaning the last small insects.',
    ],
  },
  {
    month: 5, // June

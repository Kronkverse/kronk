  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (cancelled) return;
        setAccount(acctRes);
        // Also seed Redux so downstream components (FollowButton,
        // ProfileViewerActions) that read from `state.accounts` see
        // this account. The classic profile does this via its own
        // reducers; the shelved profile fetches locally and needs
        // to relay the account explicitly. Also kick off the

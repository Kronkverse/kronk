...
const ActivityItem: React.FC<ActivityItemProps> = ({ statusId, interactions }) => {
  ...
  return (
    <div className="friends-activity-item">
      ...
      <StatusContainer
        id={statusId}
        contextType="friends_activity"
      />
    </div>
  );
};

const InteractionBar: React.FC<{ grouped: Record<string, string[]> }> = ({ grouped }) => {
  return (
...
const InteractionGroup: React.FC<{ type: string; accountIds: string[] }> = ({ type, accountIds }) => {
  const accounts = useAppSelector((state) => {
    return accountIds.map((id) => state.accounts.get(id)).filter(Boolean) as AccountRecord[];
  });

  if (accounts.length === 0) return null;

  const names: string[] = accounts.map((a) => a.get("display_name") || a.get("username")).filter(Boolean);
    ...
    <div className="friends-activity-item__interaction-group">
      <span className="friends-activity-item__avatars">
        {accounts.slice(0, 3).map((account) => (
          <span key={account.get("id")} className="friends-activity-item__avatar">
            <Avatar account={account.toJS() as Pick<Account, "id" | "acct" | "avatar" | "avatar_static">} size={18} />
          </span>
        ))}
      </span>

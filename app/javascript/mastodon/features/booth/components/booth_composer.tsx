...
// by the Ж bubble at /hub/booth/composer (legacy /hub/booth/new
// resolves to the same overlay). Was `<UploadForm>` — a bespoke
// full-panel form rendered inside Booth's SPA — until 2026-08-12.
//
// Mid-flight the composer body swaps from the field stack to a
// progress panel (audio → cover → save) with an inline
// cancel-upload affordance; the shell's own Cancel is disabled while
// submitting so a stray tap can't leave a half-uploaded set in
// limbo.

const messages = defineMessages({
  shellLabel: {
    id: 'booth.upload.shell_label',
    defaultMessage: 'Upload a set',
  },
  submitting: {
    id: 'booth.upload.submitting',
...
interface Props {
  onCancel: () => void;
  // Fires after a successful upload + create. Parent decides where
  // to navigate — Booth's directory pushes the fresh set into its
  // list and returns to `/hub/booth`.
  onCreated: (set: BoothSet) => void;
}
...
export const BoothComposer: React.FC<Props> = ({ onCancel, onCreated }) => {
  const intl = useIntl();
  ...
  const canSubmit = Boolean(
    title.trim() && artistName.trim() && audioFile !== null,
  );

  return (
    <ComposeShell
      korner='booth'
      label={intl.formatMessage(messages.shellLabel)}

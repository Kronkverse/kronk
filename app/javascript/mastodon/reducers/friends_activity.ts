...
interface FriendsActivityAction {
  ...
  items?: Array<{
    statusId: string;
    interactions: Array<{ type: string; accountId: string; created_at: string }>;
  }>;
  hasMore?: boolean;
  maxId?: string;
  error?: unknown;
...
export default function friendsActivity(state = initialState, action: FriendsActivityAction) {
  switch (action.type) {
    ...
    case FRIENDS_ACTIVITY_EXPAND_SUCCESS:
      return state.withMutations((map) => {
        ...
        if (action.maxId) {
          map.update("items", (list) => (list as ImmutableList<unknown>).concat(newItems));
        } else {

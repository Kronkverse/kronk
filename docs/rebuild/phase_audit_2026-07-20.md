...
> phase *definitions* remain authoritative; its inline statuses are from
...
| 10 InFlow/Marketplace/Skeleton | mostly | Marketplace fully shipped. **10.1** Kosmic does NOT project into the feed (scheduler writes a row, no Status; `Inflow::PublishKosmicUpdate` doesn't exist; visible card is a client banner). **10.5** no `tree.yaml`; Skeleton shipped as a Kommons sub-feature, not behind a flag |
...
| 12 Nav chrome | **shipped** | wordmark, three-way switcher (Me/Home/Nudges), Ӂ menu, mobile tab-bar, korner sub-bar. Label sets differ from plan wording |

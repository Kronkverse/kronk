...
> shipped surfaces. **Tier 3 is product *vision* from the space docs** — the
...
| Item | Evidence | Effort |
|---|---|---|
| `adding_a_korner.md` has 3 false statements: Hub "does not exist" (it does), a `hub_registered` manifest field (doesn't exist), "Kuestions has no separate table" (Question/Answer tables shipped) | `docs/korners/adding_a_korner.md:465,471,490` | S |
| `anatomy.md` self-stale — describes retired `planets.tsx`/`--space-color` as current | `docs/korners/anatomy.md:106,174` | S |
| Spec §13 lists already-resolved items as open (token generation shipped, manifest doctor-gating shipped) | `docs/kronk_korner_spec.md:427,432` | S |

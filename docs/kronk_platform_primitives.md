...
| `<AttachmentPicker>` (deferred, Phase 2b)      | Modal for picking a target korner + record. Blocked on a shared candidates search endpoint. Callers wire ad-hoc pickers via `useAttachments.addLink` in the meantime.                                                                                 | Planned at `components/attachment_picker.tsx`. Spec §4.3.                                                                             |

...
- ◇ **Rotating title (opt-in).** A korner that wants the `/home` "rotating scope" title — chevron-flanked, tap/arrow-key to cycle — sets `header.rotator: true` in its manifest. `<AutoSpaceHeader>` then renders as `<ScopeTitle>` cycling through `views:`, and each view entry may supply its own `tagline` (per-face) which falls back to the korner-level `tagline` when absent. The rotator IS the title — this stays on-standard because the Frame still owns the one `<h1>`; the korner's index must not render its own rotator, chevron, or replacement `<h1>` on top. See `AutoSpaceHeader` for the wiring and `config/korners/albutts.yaml` for a shipped example.

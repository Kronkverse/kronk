...
| 2     | React primitives (`useAttachments`, `<AttachmentSection>`, `<AttachmentPicker>`). Add rows to `docs/kronk_platform_primitives.md`. Wire the first adopter: Kalendar composer's "Create a companion album" checkbox writes an attachment (kind=spawn, trigger=field:spawn_album) instead of the hardcoded `spawn_album` field.                                      | Primitives in play; Kalendar is the reference |
| 3     | Backfill existing `album.event_id` + `booth_set.event_id` rows into `korner_attachments` (spawn/link accordingly). Retire `config/initializers/albutts_event_bus.rb` — its subscriber becomes a factory registration. Keep the FK columns for one release cycle as a passive mirror; drop them in a follow-up PR after any consumers of the FK have been migrated. | Existing pairs unified                        |

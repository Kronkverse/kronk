...
| 2b    | `<AttachmentPicker>` modal + a shared candidates-search API endpoint (`GET /api/v1/attachments/candidates?korner=<slug>&q=`) or a `search_endpoint:` manifest field. Deferred behind Phase 2a because it needs cross-korner search infra that doesn't exist yet.                                                                                                                                                                                                                                                                                                            | Attach flow surfaced            |
| 3     | Backfill existing `album.event_id` + `booth_set.event_id` rows into `korner_attachments` (spawn/link accordingly). Retire `config/initializers/albutts_event_bus.rb` — its subscriber becomes a factory registration. Wire the first source-side opt-in: Kalendar composer's "Create a companion album" checkbox writes an attachment (kind=spawn, trigger=field:spawn_album) instead of setting `event.spawn_album` directly. Keep the FK columns for one release cycle as a passive mirror; drop them in a follow-up PR after any consumers of the FK have been migrated. | Existing pairs unified          |

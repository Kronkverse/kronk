...
RSpec.describe 'Accounts Profile Sections API' do
  ...
  # Timeline shelf on the owner's profile — the simplest shape that
  # exercises the visibility gate. Kind + settings don't matter beyond
  # 'this returns account.statuses'.
  let!(:section) { Fabricate(:profile_section, account: owner, section_type: 'timeline', position: 0) }

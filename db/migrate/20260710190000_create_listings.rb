...
# Wachuneed (originally shipped as `marketplace`, renamed 2026-07-21)
# greenfield — reference korner built to spec §5 from day one. Uses
# `status_id` (§5.5 canonical linkage), snowflake IDs, and the
# spaces/wachuneed/ media prefix per storage discipline.
class CreateListings < ActiveRecord::Migration[8.0]
  def change
    create_table :listings do |t|
      ...
      t.text       :description
      t.string     :category, null: false           # creation | goods | service (renamed from `marketplace` 2026-07-21)
